...
describe("MessagePreview", () => {
  ...
  describe("rendering", () => {
    it("should render formatted Slack message", () => {
      ...
      const messageContent = screen.getByRole("region", { name: "Slackメッセージ内容" });
      expect(messageContent).toHaveTextContent("■店名：ラーメン太郎");
      expect(messageContent).toHaveTextContent("■場所：東京都渋谷区（https://example.com/ramen-taro）");
      ...
      expect(messageContent).toHaveTextContent("とても美味しいラーメンでした。また来たいです。");
    });
  ...
  describe("copy functionality", () => {
    ...
    it("should copy message to clipboard when copy button is clicked", async () => {
      const mockWriteText = vi.spyOn(globalThis.navigator.clipboard, "writeText");
  ...
  describe("accessibility", () => {
    ...
    it("should have copy button with appropriate aria-label", () => {
      ...
      const copyButton = screen.getByRole("button", { name: /コピー/ });
      expect(copyButton).toHaveAttribute("aria-label", expect.stringContaining("コピー"));
    });
